...
def generate_report(output_dir=".", log_file="qpe_histograms_log.txt", html_file="qpe_output_report.html"):
    ...
    <body>
    <h1>Quantum Phase Estimation - Comprehensive Report</h1>
    """
    ...
    for line in lines:
        ...
        if len(parts) == 5:
            filename, phase, ancilla, bitstring, est_phase = parts
            circuit_filename = f"circuit_general_phase_{round(float(phase), 3)}_a{ancilla}.png"
    ...
    for line in lines:
        ...
        if parts[0] == "exact phase":
            _, filename, t, ancilla, bitstring, expected_bin, tensor, simplified, matrix = parts
            circuit_filename = f"circuit_exact_phase_t{round(float(t), 4)}_a{ancilla}_k{int(bitstring, 2)}.png"
            img_path = output_path / filename
            circuit_path = output_path / circuit_filename
            if not added_ham:
                ham_block = f"""
    ...
    for line in lines:
        ...
        if parts[0] == "general case":
            (_, filename, time, shots, ancilla, bitstring, est_phase,
             ...
             tensor, simplified, matrix, expected_bitstring) = parts
            circuit_filename = f"circuit_general_case_t{time}_shots{shots}_a{ancilla}.png"
            unit_circle_filename = f"unit_circle_t{time}_shots{shots}_a{ancilla}.png"
            circuit_path = output_path / circuit_filename
